...
void publishImuData(void)
{
    ...
    if(!m_e2box_imu.data_acquisition){
        ...
        imu_data.header.frame_id = "imu_e2box_link";
        ...
        */
        ...
        imu_data.orientation.y = m_e2box_imu.m_dQuaternion[2];
        imu_data.orientation.x = m_e2box_imu.m_dQuaternion[1];
        imu_data.orientation.z = -m_e2box_imu.m_dQuaternion[0];
        imu_data.orientation.w = m_e2box_imu.m_dQuaternion[3];

        // imu_data_temp
        imu_data_temp.angular_velocity.y = m_e2box_imu.m_dAngRate[0]*M_PI/180.0;
        imu_data_temp.angular_velocity.x = m_e2box_imu.m_dAngRate[1]*M_PI/180.0;
        imu_data_temp.angular_velocity.z = -m_e2box_imu.m_dAngRate[2]*M_PI/180.0;

        imu_data_temp.linear_acceleration.y = m_e2box_imu.m_dAccel[0]*9.80665;
        imu_data_temp.linear_acceleration.x = m_e2box_imu.m_dAccel[1]*9.80665;
        imu_data_temp.linear_acceleration.z = -m_e2box_imu.m_dAccel[2]*9.80665;

        // imu_data_prev
        imu_data_prev.angular_velocity.y = imu_data_temp.angular_velocity.x;
        imu_data_prev.angular_velocity.x = imu_data_temp.angular_velocity.y;
        imu_data_prev.angular_velocity.z = -imu_data_temp.angular_velocity.z;

        imu_data_prev.linear_acceleration.y = imu_data_temp.linear_acceleration.x;
        imu_data_prev.linear_acceleration.x = imu_data_temp.linear_acceleration.y;
        imu_data_prev.linear_acceleration.z = -imu_data_temp.linear_acceleration.z;
...
int main(int argc, char *argv[])
{
    ...
    nh.param("init", init, false);
    nh.param("yaw0", yaw0, false);
    ...
    if(init){
        unsigned char quaternion_init[] = "<sof1>";
        unsigned char angular_init[] = "<sog1>";
        unsigned char accel_init[] = "<soa2>";

        m_e2box_imu.serial.Write(quaternion_init, 7);
        m_e2box_imu.serial.Write(angular_init, 7);
        m_e2box_imu.serial.Write(accel_init, 7);
    }

    if(yaw0){
        unsigned char yaw0_init[] = "<cmoz>";
        m_e2box_imu.serial.Write(yaw0_init, 7);
    }

    imu_pub = nh.advertise<sensor_msgs::Imu>("imu_e2box", 100);
