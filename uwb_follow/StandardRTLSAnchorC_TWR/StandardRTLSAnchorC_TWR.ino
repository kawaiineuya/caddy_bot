...
ros::NodeHandle nh;
std_msgs::Float32 uwb_c_range_msg;
std_msgs::Float32 uwb_c_rx_power_msg;
ros::Publisher uwb_c_range("uwb_c_range", &uwb_c_range_msg);
ros::Publisher uwb_c_rx_power("uwb_c_rx_power", &uwb_c_rx_power_msg);
...
void setup() {
    ...
    nh.advertise(uwb_c_range);
    nh.advertise(uwb_c_rx_power);
...
void loop() {
     ...
     if(result.success) {
        ...
        if (nh.connected()){
            ...
            uwb_c_range_msg.data = range_self;
            uwb_c_rx_power_msg.data = DW1000Ng::getReceivePower();
            uwb_c_range.publish(&uwb_c_range_msg);
            uwb_c_rx_power.publish(&uwb_c_rx_power_msg);
            // Serial.print("ok");
        }else{
